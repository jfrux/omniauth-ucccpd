...
describe OmniAuth::Strategies::UCCCPD do
  def app
    lambda do |_env|
      [200, {}, ['Hello.']]
    end
  end
  let(:fresh_strategy) { Class.new(OmniAuth::Strategies::UCCCPD) }
  ...
  describe '#client' do
    ...
    it 'is initialized with symbolized client_options' do
      instance = subject.new(app, :client_options => {'authorize_url' => 'http://test.ccpd.uc.edu'})
      expect(instance.client.options[:authorize_url]).to eq('http://test.ccpd.uc.edu')
    end
...
describe OmniAuth::Strategies::UCCCPD::CallbackError do
  let(:error) { Class.new(OmniAuth::Strategies::UCCCPD::CallbackError) }
  describe '#message' do
    subject { error }
